...
- `apps/web` — public acquisition website, quote intake, attribution and website chat.
...
- `packages/ui` — shared Car Log design system.
- `supabase/migrations` — PostgreSQL schema migrations.
- `infrastructure/docker` — production containers and Compose topology.
- `infrastructure/nginx` — reverse proxy configuration.
- `infrastructure/hostinger` — Hostinger VPS deployment and migration scripts.

## Runtime topology

Production deployment is performed only from the `dev` branch by GitHub Actions. Validation must pass before deployment. The deployment job connects to the Hostinger VPS, updates the existing `dev` checkout with a fast-forward-only pull, applies migrations, builds and starts the Docker Compose services, installs the Nginx configuration, validates Nginx and performs health checks.

Production services:

- `carlogconnection.com` → `apps/web` on the web container.
...
No alternative deployment platform is part of the canonical runtime.
...
- CMS owns website pages, navigation, media and publishing state.
...
Dialpad is the canonical telephony and SMS provider. Website chat is a native channel. Social, advertising, transportation, compliance, finance and Google products are modeled as separate provider capabilities instead of leaking provider semantics into the domain.
...
The Unified Inbox normalizes website chat, Dialpad SMS, Dialpad calls and supported social channels into conversations. A conversation may be linked to a contact, customer, lead, quote, order or carrier. Website context preserves visitor/session identity, current page, referrer and campaign identifiers such as UTM parameters, GCLID, FBCLID and TTCLID.
