...
The Car Log repository must not depend on, embed, redirect to, proxy, or deploy against the deprecated public hostname previously used by the project.
...
GitHub Actions and the architecture audit reject the deprecated hostname if it is reintroduced. GitHub Pages remains a read-only Admin OS preview and does not represent a public marketing website.
