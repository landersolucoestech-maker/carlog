...
function ssl(){if(process.env.DATABASE_SSL==='disable')return false;if(process.env.NODE_ENV!=='production'&&process.env.DATABASE_SSL!=='require')return undefined;return{rejectUnauthorized:process.env.DATABASE_SSL_REJECT_UNAUTHORIZED!=='false'}}
export const pool=new Pool({connectionString,max:Number(process.env.DATABASE_POOL_MAX??5),ssl:ssl()});
