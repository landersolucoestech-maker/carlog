...
const allowedOrigins = new Set(
  [process.env.PUBLIC_WEB_URL, process.env.ADMIN_URL].filter((value): value is string => Boolean(value)),
);
...
app.addHook('onRequest', async (request, reply) => {
  ...
  if (origin && allowedOrigins.has(origin)) {
    ...
    reply.header('access-control-allow-headers', 'Authorization,Content-Type,Idempotency-Key,X-Correlation-Id,X-Chat-Token');
    reply.header('access-control-allow-methods', 'GET,POST,PUT,PATCH,DELETE,OPTIONS');
  }
  if (request.method === 'OPTIONS') {
    if (origin && !allowedOrigins.has(origin)) return reply.code(403).send({ error: 'Origin not allowed' });
    return reply.code(204).send();
  }
});

app.setErrorHandler((error, request, reply) => {
  const candidateStatus = typeof (error as { statusCode?: unknown }).statusCode === 'number'
    ? (error as { statusCode: number }).statusCode
    : 500;
  const status = error instanceof ZodError ? 400 : candidateStatus;
  const correlationId = String(reply.getHeader('x-correlation-id') ?? requestCorrelationId(request));
  const message = error instanceof Error ? error.message : 'Request failed';
  if (status >= 500) request.log.error({ err: error, correlationId }, 'Request failed');
