...
export async function registerCommunicationRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/v1/conversations/:id/timeline', async (request) => {
    ...
    const [messages,calls] = await Promise.all([query(`select * from app.messages where conversation_id=$1 order by occurred_at,id`, [id]),query(`select * from app.calls where conversation_id=$1 order by coalesce(started_at,created_at),id`, [id])]); return { conversationId: id, messages, calls };
  });
  app.patch('/v1/conversations/:id', async (request) => {
    ...
    return transaction(async (client) => { const before = await client.query(`select * from app.conversations where id=$1 for update`, [id]); if (!before.rows[0]) throw Object.assign(new Error('Conversation not found'), { statusCode: 404 }); const updated = await client.query(`update app.conversations set status=coalesce($2,status),assigned_user_id=case when $3::boolean then $4::uuid else assigned_user_id end,priority=coalesce($5,priority),tags=case when $6::boolean then $7::text[] else tags end,updated_at=now() where id=$1 returning *`, [id, body.status ?? null, Object.prototype.hasOwnProperty.call(body,'assignedUserId'), body.assignedUserId ?? null, body.priority ?? null, Object.prototype.hasOwnProperty.call(body,'tags'), body.tags ?? []]); await recordAudit(client, { actorUserId: user.id, action: 'conversation.updated', entityType: 'conversation', entityId: id, before: before.rows[0], after: updated.rows[0], correlationId }); return updated.rows[0]; });
  });
  app.post('/v1/conversations/:id/messages', async (request, reply) => {
    ...
    const result = await transaction(async (client) => {
      ...
      const target = delivery(row.channel); let connectionId: string | null = null;
      if (target) { const connection = await client.query<{id:string}>(`select ic.id from app.integration_connections ic join app.integration_capabilities cp on cp.connection_id=ic.id and cp.capability=$2 and cp.authorized=true where ic.provider=$1 and ic.status='connected' order by ic.updated_at desc limit 1`, [target.provider, target.capability]); if (!connection.rows[0]) throw Object.assign(new Error(`${target.provider} is not connected for ${target.capability}`), { statusCode: 409 }); connectionId=connection.rows[0].id; }
      ...
      if (target) await enqueueIntegrationAction(client, { connectionId, provider: target.provider, capability: target.capability, operation: target.operation, payload: { conversationId: id, messageId, body: body.body, contactId: row.contact_id }, idempotencyKey: body.idempotencyKey, correlationId });
      await client.query(`update app.conversations set last_activity_at=now(),updated_at=now() where id=$1`, [id]); await enqueueDomainEvent(client, { type: 'message.sent', source: 'communication', correlationId, idempotencyKey: `message.sent:${messageId}`, actorUserId: user.id, payload: { conversationId: id, messageId, queued: Boolean(target) } }); await recordAudit(client, { actorUserId: user.id, action: 'message.created', entityType: 'message', entityId: messageId, correlationId, metadata: { conversationId: id, channel: row.channel } }); return { messageId, status: target ? 'queued' : 'sent' };
